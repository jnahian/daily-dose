...
- `npm run team:migrate-members -- <source-team> <target-team> [options]` - Move all active members from one team to another (the `--` separator is required so npm forwards the arguments to the script)
...
- Use dayjs for all date operations with timezone support — on the frontend too, via `web/src/utils/adminFormat.ts` (see "Calendar dates vs instants" under Project-Specific Conventions)
