  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3 mb-6">
        <div
          className={`p-3 bg-bg-primary rounded-lg border border-border-default`}
        >
          <Icon size={24} className={script.iconColor} />
        </div>
        <h3 className="text-xl md:text-2xl font-bold text-text-primary break-words">
          {script.name}
        </h3>
      </div>
      ...
      {script.usageNote && (
        <div
          className={`p-4 rounded-lg border mb-6 ${noteStyles[script.usageNote.type]}`}
        >
              ...
              <tbody className="text-text-secondary">
                {script.parameters.map((param, index) => (
                  <tr key={index} className="border-b border-border-default">
                    <td className="py-2 pr-4 font-mono text-brand-cyan">
                      {param.name}
                    </td>
        ...
        <div className="mb-6">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            {script.features.map((feature, index) => (
              <div
                key={index}
                className="border border-border-default p-4 rounded-lg bg-bg-primary"
              >
                <h5 className="font-semibold text-brand-cyan mb-2">
                  {feature.title}
                </h5>
