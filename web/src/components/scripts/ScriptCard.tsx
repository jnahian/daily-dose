...
import { Database, Users, Play, Bug, Settings, Sprout, UserCheck, Send, Info, Cog, AlertTriangle, AlertCircle, CheckCircle, XCircle } from 'lucide-react';
...
export const ScriptCard = ({ script }: ScriptCardProps) => {
    ...
    return (
        <div id={`script-${script.id}`} className="bg-brand-navy-light rounded-xl border border-white/10 p-8 mb-8 scroll-mt-24">
            {/* Header */}
            ...
            {script.usageNote && (
                <div className={`p-4 rounded-lg border mb-6 ${noteStyles[script.usageNote.type]}`}>
                    <div className="flex items-start gap-3">
                        {React.createElement(noteIcons[script.usageNote.type], { size: 20, className: 'mt-0.5' })}
                        <div>
