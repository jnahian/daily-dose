...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function performSearch(query) {
        ...
        if (!query.trim()) {
            ...
            sections.forEach(section => {
                ...
                section.innerHTML = content.replace(/<span class="search-highlight">(.*?)<\/span>/gi, '$1');
            });
        ...
        if (activeLink) {
            ...
            for (let i = sections.length - 1; i >= 0; i--) {
                ...
                if (section.offsetTop <= scrollPos) {
                    const correspondingLink = document.querySelector(`.nav-item[href="#${section.id}"]`);
    ...
    document.addEventListener('keydown', function(e) {
        ...
        if ((e.ctrlKey || e.metaKey) && e.key === 'f') {
            e.preventDefault();
            const activeSearch = window.innerWidth >= 640 ? searchInput : mobileSearchInput;
            if (activeSearch) {
                activeSearch.focus();
                activeSearch.select();
            }
    ...
    if (window.location.hash) {
        ...
        if (targetElement) {
            setTimeout(() => {
                ...
                const correspondingLink = document.querySelector(`.nav-item[href="#${targetId}"]`);
                if (correspondingLink) {
