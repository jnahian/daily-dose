...
class SlackManifestManager {
  constructor() {
    ...
    if (!this.token && !this.refreshToken) {
      console.error(
        "❌ Error: SLACK_USER_TOKEN or SLACK_USER_REFRESH_TOKEN not found in environment variables"
      );
      console.error(
        "   You need a user token with apps:write scope to manage app manifests"
      );
      process.exit(1);
    }
    ...
    if (this.refreshToken && (!this.clientId || !this.clientSecret)) {
      console.error(
        "❌ Error: SLACK_CLIENT_ID and SLACK_CLIENT_SECRET are required when using refresh token"
      );
      process.exit(1);
    }
  }
  ...
  loadManifest() {
    try {
      ...
      console.error(
        `❌ Error loading manifest from ${this.manifestPath}:`,
        error.message
      );
      process.exit(1);
    }
  ...
  /**
   * Make API request to Slack with automatic token refresh
   */
  ...
  async createApp(manifest, dryRun = false) {
    ...
    try {
      ...
      console.log(
        "   3. Update your manifest URLs to point to your actual domain"
      );
  ...
  async updateApp(appId, manifest, dryRun = false) {
    ...
    try {
      ...
      if (result.permissions_updated) {
        console.log(
          "⚠️  Permissions were updated - you may need to reinstall the app"
        );
      }
  ...
  async getCurrentManifest(appId) {
    try {
      ...
      console.warn(
        "⚠️  Could not fetch current manifest for comparison:",
        error.message
      );
      return null;
    }
  ...
  validateManifest(manifest) {
    ...
    if (missing.length > 0) {
      throw new Error(
        `Missing required manifest fields: ${missing.join(", ")}`
      );
    }
    ...
    if (manifestStr.includes("your-domain.com")) {
      console.warn(
        "⚠️  Warning: Manifest contains placeholder URLs (your-domain.com)"
      );
      console.warn(
        "   Make sure to update these with your actual domain before deploying"
      );
    }
