...
- Standup reminder and followup DMs ignored the `TeamMember.receiveNotifications` opt-out (`/dd-standup-reminder notify=off`): `schedulerService.sendStandupReminders` / `sendFollowupReminders` filtered only by `role !== "ADMIN"`, so opted-out members were still DM'd every day. Both filters now also require `receiveNotifications`. Added `test/services/schedulerService.test.js`. (#18) (`src/services/schedulerService.js`)
