...
- Zoho People leave & holiday integration: a nightly cron job (`schedulerService`, default `30 1 * * *`, override with `ZOHO_SYNC_CRON`) syncs approved leave and org holidays from Zoho People into the existing `Leave`/`Holiday` tables, tagged with a new `source`/`externalId` pair (`MANUAL` vs `ZOHO`) so synced rows dedupe cleanly on re-sync without disturbing manually-entered leaves/holidays. Since gating (`standupService.getActiveMembers`, `postTeamStandup`'s "On Leave" block) already reads from those same tables, no changes were needed there — Zoho-sourced leave automatically skips reminders and shows up in the "out today" summary line, and a synced holiday skips the whole standup run, same as a manually-entered one.
  ...
  - `ZohoSyncRun` records `skippedUnmapped` / `skippedNotApproved` / `skippedInvalid` alongside `recordsSynced`, and `/dd-zoho-sync-status` surfaces them. Previously a misconfigured integration (employee IDs that don't match what Zoho returns, or response field names the mappers don't recognize) recorded `status: SUCCESS, recordsSynced: 0` — indistinguishable from a night with nothing to sync. A zero-record run with skips now renders an explicit warning pointing at `/dd-zoho-map-list` or the field-name mismatch. (`prisma/migrations/20260728120000_add_zoho_sync_run_skip_counters`, `src/services/zoho/zohoSyncService.js`, `src/commands/zoho.js`)
  - `/dd-zoho-sync-status` resolves the latest run per type with one `findFirst` each instead of deriving it from a `take: 10` list, so a burst of runs of one type can no longer push the other out of the window and make it read as "never synced". Failed runs no longer render the raw thrown message (which can be a Prisma error) — they point to the server logs instead, per `errorHelper`'s policy.
  - The `ZohoSyncRun` audit write no longer masks the underlying Zoho error if the audit insert itself fails.
